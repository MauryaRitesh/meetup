...
def initialize():
    try:
        ...
        print("Please create a valid config file in order to log in.\n" + "Exiting...\n")
        sys.exit(1)
...
def gather_event_links(driver):
    ...
    event_links = [
        href.get_attribute("href") for href in driver.find_elements_by_xpath("//a[@class='resetLink big event wrapNice omnCamp omngj_sj7emga omnrv_fe1mga ']")]
...
def rsvp_event(driver):
    try:
        driver.find_element_by_xpath("//span[@class='text--body inverted']//span")
        return True
    except:
        pass

    event_name = driver.find_element_by_xpath("//h1[@class='pageHead-headline text--pageTitle']").text
    
    event_date = driver.find_element_by_xpath("//time[@class='eventStatusLabel']//span").text
